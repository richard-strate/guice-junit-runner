...
public class GuiceJUnitRunner extends BlockJUnit4ClassRunner {
    ...
    private Injector createInjectorFor(final Collection<Class<? extends Module>> classes) throws InitializationError {
        ...
        return Guice.createInjector(Collections2.transform(classes, (final Class<? extends Module> module) -> {
            try {
                return module.getConstructor().newInstance();
            } catch (NoSuchMethodException | SecurityException | InstantiationException | IllegalAccessException | IllegalArgumentException | InvocationTargetException ex) {
                throw new RuntimeException(String.format("Failed to create guice module: '%s'", module.getClass().getName()), ex);
            }
        }));
    }
